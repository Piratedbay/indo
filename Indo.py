...
def masuk():
    ...
    if __Kontol__Jimin__Buriq__ in(""):
        ...
        jalan("\n%s[%s!%s] Ingat, Harus Pakai Jai Tumbal !!"%(P,K,P))
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n%s[%s•%s] Jai %sOK%s saved >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta)
                            print '%s[%s•%s] Jai %sCP %ssaved > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta)
...
def menu():
    ...
    print('[%s8%s] Cek Opsi Jai Checkpoin'%(H,P));time.sleep(0.03)
    ...
    if __Aang__Sayang__Laura__ == '':
    	...
    	print "\n%s[%s1%s] Hasil crack Jai facebook "%(P,K,P)
...
def hasill(__Aang__Sayang__Laura__):
	if __Aang__Sayang__Laura__ in[""]:
		...
		try:
			...
			print("%s[%s•%s] Hasil Crack Jai Facebook"%(P,K,P));jeda(2)
			nm_file = ("%s"%(file)).replace("-", " ")
			jalan("[%s•%s] Total Jai : %s"%(K,P,len(total)))
